 //通用接口地址设置  
 //  183.26.245.185
var server_url = "http://183.26.247.151:8080/";  
...
function getTrueUserName(){
	var userData=plus.storage.getItem("userData");
	var userTrueName= JSON.parse(userData).userTrueName;
	console.log("userTrueName:"+userTrueName);
	return userTrueName;
}
function getUserDescription(){
	var userData=plus.storage.getItem("userData");
	var userDescription= JSON.parse(userData).userDescription;
	console.log("userDescription:"+userDescription);
	return userDescription;
}
function getUserUserName(){
	var userData=plus.storage.getItem("userData");
	var userUserName= JSON.parse(userData).userUserName;
	console.log("userUserName:"+userUserName);
	return userUserName;
}
function getUserEmail(){
	var userData=plus.storage.getItem("userData");
	var userEmail= JSON.parse(userData).userEmail;
	console.log("userEmail:"+userEmail);
	return userEmail;
}
function getUserPhone(){
	var userData=plus.storage.getItem("userData");
	var userPhone= JSON.parse(userData).userPhone;
	console.log("userPhone:"+userPhone);
	return userPhone;
}
function getUserSex(){
	var userData=plus.storage.getItem("userData");
	var userSex= JSON.parse(userData).userSex;
